...
1. **網路連接**: 容器共享主機的網路命名空間，可以直接訪問網路
2. **DNS 解析**: 容器已配置 DNS（使用 Google DNS 8.8.8.8 和 8.8.4.4）
3. **持久化**: 使用 OverlayFS 時，每個容器的修改（包括安裝的套件）是隔離的
4. **權限**: apt-get 操作在容器內以 root 身份運行（已通過 user namespace 映射）

### 重建基礎映像

如果您在更新程式碼後需要重建基礎映像（例如更新 apt-get 配置），請執行：
